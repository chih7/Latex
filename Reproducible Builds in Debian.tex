...
%                 USTC Linux User Group <lug@ustc.edu.cn>
%
% Build HOWTO
% -----------
% * xelatex
...
\only<1>{%
  {\Large \texttt{diffoscope}：深入压缩包与文件的\texttt{diff}增强版}
  
  \vspace{1em}
  由reproducible-builds团队领衔开发，专门用来探查\texttt{.deb}包之间的差异究竟在哪里，便于对症下药。%
  
  \vfill%
  % include png: diffoscope_example.png
  \begin{figure}[htbp]%
  \includegraphics[width=.95\paperwidth]{diffoscope_example.png}%
  \end{figure}%
}%
\end{frame}
...
\note{展示一个使用\texttt{gbp buildpackage}构建的软件包，例如QEverCloud}
\begin{enumerate}
\item 比较散列值即可；
